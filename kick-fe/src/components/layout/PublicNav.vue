  ...
  >
      ...
      <div class="flex items-center gap-1 shrink-0">
        <template v-if="isAuthenticated">
          ...
          <button
            v-if="!isMobile"
            type="button"
            class="relative w-10 h-10 flex items-center justify-center rounded-lg text-black/70 hover:text-black hover:bg-black/5 transition-all duration-200 border-0 bg-transparent cursor-pointer"
          ...
          >
            ...
            <span
              v-if="unreadCount"
              class="absolute top-1 right-1 min-w-4 h-4 px-1 rounded-full bg-black text-white text-[10px] font-bold flex items-center justify-center leading-none shadow-sm"
              >{{ unreadCount }}</span
            >
          </button>
          ...
          <RouterLink
            v-if="!isMobile"
            to="/wishlist"
            class="relative w-10 h-10 flex items-center justify-center rounded-lg text-black/70 hover:text-black hover:bg-black/5 transition-all duration-200 no-underline"
          ...
          <RouterLink
            to="/account"
            class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-black/5 transition-all duration-200 ml-1 no-underline"
        ...
        <template v-else>
          ...
          <div v-if="!isMobile" class="flex items-center gap-1.5 ml-1">
            <RouterLink
              to="/login"
              class="inline-flex items-center h-9 px-3.5 text-xs font-bold tracking-wider uppercase text-black bg-white border border-neutral-200 hover:bg-neutral-50 rounded-lg transition-all duration-200 active:scale-95 no-underline"
            ...
            <RouterLink
              to="/register"
              class="inline-flex items-center h-9 px-3.5 text-xs font-bold tracking-wider uppercase text-white bg-black hover:bg-neutral-800 rounded-lg transition-all duration-200 active:scale-95 no-underline"
              aria-label="Đăng ký"
            >
              Đăng ký
            </RouterLink>
    ...
    >
      <div
        ...
        class="w-full bg-white border-t border-black/10 py-3 px-6 shadow-lg"
        role="navigation"
      ...
      >
        ...
        <div v-if="!isAuthenticated" class="mt-6 flex flex-col gap-2">
          <RouterLink
            to="/login"
            class="block w-full py-3 text-center text-xs font-bold tracking-wider uppercase text-black bg-white border border-neutral-200 hover:bg-neutral-50 rounded-lg no-underline transition-all duration-200"
            @click="menuOpen = false"
          >
            Đăng nhập
          </RouterLink>
          <RouterLink
            to="/register"
            class="block w-full py-3 text-center text-xs font-bold tracking-wider uppercase text-white bg-black hover:bg-neutral-800 rounded-lg no-underline transition-all duration-200"
            @click="menuOpen = false"
          >
            Đăng ký
          </RouterLink>
